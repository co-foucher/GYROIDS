...
This is the Fischer-Koch S TPMS type, built on top of TPMSModel (see
tpms_base.py). All the grid validation / field pipeline / meshing / export
logic lives in the base class; this subclass only supplies the Fischer-Koch
S's implicit surface equation.

# =====================================================================================================================
USE EXAMPLE
-----------
>>> model = FischerKochSModel(x, y, z, px, py, pz, thickness)
...
# 1) FischerKochSModel
# =====================================================================
class FischerKochSModel(TPMSModel):
    """
    ============================================================================
    1) FISCHERKOCHSMODEL
    Represents a Fischer-Koch S (F-KS) TPMS scalar field defined on a 3D grid.
    ============================================================================
    ...
    NOTES
    -----
    - The Fischer-Koch S surface is a classic TPMS with a cyclic combination
      of double-frequency and single-frequency trig terms. Its implicit
      equation is:

          F(x, y, z) = cos(2X)sin(Y)cos(Z) + cos(2Y)sin(Z)cos(X)
                     + cos(2Z)sin(X)cos(Y)

      where X = 2π/px·x, Y = 2π/py·y, Z = 2π/pz·z. The field amplitude range
      is roughly [-1.41, +1.41] (±√2).
    - `load` is inherited from TPMSModel and returns a FischerKochSModel
      instance.
    ...
    >>> model = FischerKochSModel(x, y, z, px, py, pz, thickness)
    >>> field = model.compute_field()
    >>> model = FischerKochSModel.load("gyroid_data.npz")
    """
...
# 3) create_a_fischer_koch_s
# =====================================================================
def create_a_fischer_koch_s(x: np.ndarray,
                            y: np.ndarray,
                            z: np.ndarray,
                            px: np.ndarray,
                            py: np.ndarray,
    ...
    ============================================================================
    3) CREATE_A_FISCHER_KOCH_S
    Convenience function to create a Fischer-Koch S model, compute the field,
    generate and simplify the mesh, and save results. Thin wrapper around
    tpms_base.create_a_tpms(FischerKochSModel, ...).
    ============================================================================
    ...
    field_mode : str, optional
        Field computation mode passed to FischerKochSModel.compute_field
        (default = "distance").
